            ...
            <div x-data="{ userMenu: false }" class="relative">
                <button @click="userMenu = !userMenu" class="flex items-center gap-3">
                    <span class="flex h-10 w-10 items-center justify-center rounded-full bg-brand-500 text-sm font-semibold text-white">
                        <?= esc(strtoupper(substr($meta['admin_name'] ?? 'A', 0, 1))) ?>
                    </span>
                    <span class="hidden text-left sm:block">
                        <span class="block text-sm font-medium text-gray-700 dark:text-gray-300"><?= esc($meta['admin_name'] ?? '-') ?></span>
                        <span class="block text-xs text-gray-400"><?= esc($meta['admin_level'] ?? '-') ?></span>
                    </span>
